...
- [x] 1. (P) Establish the `common/` module with shared cross-cutting types
- [x] 1.1 (P) Create `ErrorResponse` in the common module
...
- [x] 1.2 (P) Update `DogProfileController` to import from `common.model`
...
- [x] 2. (P) Relocate, correct, and clean up the matcher service
- [x] 2.1 (P) Move `DogMatcherService` into the `match` module and adapt it to operate on persisted dog profiles
...
- [x] 2.2 (P) Fix the three algorithm bugs in the relocated `DogMatcherService`
...
- [x] 2.3 Delete the root `service/` package
...
- [x] 3. (P) Create the `match` module domain model
- [x] 3.1 (P) Create the response DTOs for the matches endpoint
...
- [x] 3.2 (P) Create the domain exception for an unknown dog
...
- [x] 4. Implement the match orchestration service
...
- [x] 5. Implement the match retrieval controller
...
- [x] 6. Write tests
- [x] 6.1 Unit-test the matcher service algorithm
...
- [x] 6.2 Unit-test the match orchestration service
...
- [x] 6.3 `@WebMvcTest` the match controller
